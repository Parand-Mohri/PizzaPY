...
from models.post_customer import find_singe_postcode, find_customer_id
...
def create_customer():
    ...
    if not find_singe_postcode(adress.postcode):
        return make_response({"error": f"a customer with this postcode is aready exist with customer_id {find_customer_id(adress.postcode)}"})
